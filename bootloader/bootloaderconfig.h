...
static inline void  bootLoaderInit(void)
{
    ...
    if(!(MCUCSR & (1 << PORF)) && !(MCUCSR & (1 << EXTRF)))    /* If this was not a power-on reset or external reset, ignore */
	{
		MCUCSR = 0;
        leaveBootloader();
	}
...
#define bootLoaderCondition()   ((PIND & (1 << 4)) == 0) // changed to use D- pin for USnooBie
